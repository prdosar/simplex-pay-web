  ...
  return (
    <div className="p-8">
      ...
      {statsLoading ? (
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
          <StatCard label="Utilisateurs" value={stats.totalUsers} sub={`+${stats.newUsersThisWeek} cette semaine`} color="text-primary" />
          <StatCard label="Offres totales" value={stats.totalOffers} sub={`+${stats.newOffersThisWeek} cette semaine`} />
          <StatCard label="Offres actives" value={stats.activeOffers} color="text-success" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border border-border">
          <div className="px-6 py-4 border-b border-border flex items-center justify-between">
            ...
            <a href="/offers" className="text-xs text-primary hover:underline">Voir tout →</a>
          </div>
            ...
            {recentOffers?.items.map(offer => (
              <div key={offer.id} className="px-6 py-3 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">{offer.creator.firstName} {offer.creator.lastName}</p>
                </div>
              ...
              <p className="px-6 py-4 text-sm text-muted-foreground">Aucune offre</p>
            )}
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-border">
          <div className="px-6 py-4 border-b border-border flex items-center justify-between">
            ...
            <a href="/users" className="text-xs text-primary hover:underline">Voir tout →</a>
          </div>
          <div className="divide-y divide-border">
            {recentUsers?.items.map(user => (
              ...
              <p className="px-6 py-4 text-sm text-muted-foreground">Aucun utilisateur</p>
            )}
          </div>
        </div>
      </div>
