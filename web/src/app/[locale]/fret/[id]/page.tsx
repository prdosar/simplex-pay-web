...
import type { BoatShippingOfferDto, PagedResult, ReviewDto } from '@/types/api'

export default function BoatShippingDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params)
  ...
  const { data: offer, isLoading, error } = useSWR<BoatShippingOfferDto>(
    `/api/boat-shipping/${id}`,
    (url: string) => api.get<BoatShippingOfferDto>(url)
  )
      ...
      <div className="bg-white border rounded-2xl overflow-hidden" style={{ borderColor: '#e2e8f0' }}>
        <div className="p-6 text-white" style={{ background: 'linear-gradient(135deg, #0d9488, #0f766e)' }}>
          <div className="flex items-center gap-3 flex-wrap mb-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-2xl font-bold">{offer.departurePort}</span>
            </div>
            <svg className="w-6 h-6 opacity-80" viewBox="0 0 24 24" fill="currentColor">
              <path d="M20 21c-1.39 0-2.78-.47-4-1.32c-2.44 1.71-5.56 1.71-8 0C6.78 20.53 5.39 21 4 21H2v2h2c1.38 0 2.74-.35 4-.99c2.52 1.29 5.48 1.29 8 0c1.26.65 2.62.99 4 .99h2v-2h-2M3.95 19H4c1.6 0 3.02-.88 4-2c.98 1.12 2.4 2 4 2s3.02-.88 4-2c.98 1.12 2.4 2 4 2h.05l1.89-6.68c.08-.26.06-.54-.06-.78s-.34-.42-.6-.5L20 10.62V6c0-1.11-.89-2-2-2h-3V1H9v3H6c-1.11 0-2 .89-2 2v4.62l-1.29.42c-.26.08-.48.26-.6.5s-.15.52-.06.78L3.95 19M6 6h12v3.97L12 8L6 9.97V6Z"/>
            </svg>
            <div className="flex items-center gap-2">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={flagUrl(offer.destinationCountryCode)} alt="" className="w-10 h-7 rounded object-cover shadow ring-2 ring-white/40" />
              <span className="text-2xl font-bold">{offer.destinationPort}</span>
            </div>
          </div>
        ...
        <div className="p-6">
          <div className="grid grid-cols-2 gap-4 mb-6 pb-6 border-b" style={{ borderColor: '#f1f5f9' }}>
            <div>
              <p className="text-xs uppercase tracking-wide mb-1" style={{ color: '#64748b' }}>{locale === 'fr' ? 'Livres disponibles' : 'Available lbs'}</p>
              <p className="text-2xl font-extrabold" style={{ color: '#0d9488' }}>{offer.availableLbs.toLocaleString()} <span className="text-sm font-medium" style={{ color: '#64748b' }}>lbs</span></p>
            </div>
            <div>
              <p className="text-xs uppercase tracking-wide mb-1" style={{ color: '#64748b' }}>{locale === 'fr' ? 'Prix par livre' : 'Price per lb'}</p>
              <p className="text-2xl font-extrabold" style={{ color: '#0d9488' }}>{offer.pricePerLb.toLocaleString()}<span className="text-sm font-medium" style={{ color: '#64748b' }}> / lb</span></p>
            </div>
          </div>
          ...
          <div>
            <p className="text-xs uppercase tracking-wide mb-3" style={{ color: '#64748b' }}>{locale === 'fr' ? 'Expéditeur' : 'Shipper'}</p>
      ...
      <div className="mt-6 bg-white border rounded-2xl p-6" style={{ borderColor: '#e2e8f0' }}>
        ...
        {reviews && reviews.items.length > 0 ? (
          ...
          <p className="text-sm italic py-2" style={{ color: '#94a3b8' }}>
            {locale === 'fr' ? 'Aucun avis pour cet expéditeur. Sois le premier.' : 'No reviews yet. Be the first.'}
          </p>
        )}
      </div>
